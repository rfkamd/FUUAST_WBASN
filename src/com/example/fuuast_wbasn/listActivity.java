...
//	        main = new UdpBroadcast();
	        ...
	        lv.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view,
					...
					dialog.setContentView(R.layout.prompt2);
					dialog.setTitle("Values");
					
					
					
					TextView txtTemp = (TextView) dialog.findViewById(R.id.textView1);
					TextView txtTime = (TextView) dialog.findViewById(R.id.textView2);
					
					txtTemp.setText(list.get(position).get("Temp"));
...
//            mydb.rawQuery("Select IP,Location,User from "+TABLE+" Where DeviceCode = ?", new String[]{"DU"});
            ...
            //deviceList =
            deviceList = compileList(db.getTemp());
...
//	   }
	...
	public List<TempInfo> compileList(Cursor cursor)
	   {
		List<TempInfo> tempList = new ArrayList();
		 ...
		 //  list =  new ArrayList<HashMap<String><String>>();
		   ...
		   if (cursor.moveToFirst()) {
		        do {
		        	...
		        	map.put("DateTime", cursor.getString(cursor.getColumnIndex("Time")));
		        //	map.put("IP", cursor.getString(cursor.getColumnIndex("IP")));
		        	
		        	
		        	list.add(map);
		        	
		        	TempInfo dList = new TempInfo();
		        	dList.setTemp(cursor.getString(cursor.getColumnIndex("Temp")));
		           	dList.setDateTime(cursor.getString(cursor.getColumnIndex("Time")));
		        //    dList.setIPAdress((cursor.getString(cursor.getColumnIndex("IP"))));
		           
		            tempList.add(dList);
		        } while (cursor.moveToNext());
		    }
		   
		   for(TempInfo temp:tempList)
		   {
			   Log.d("rfk","ListVIEW:"+temp.getTemp()+","+temp.getDateTime());
		   }
